...
p.ba <- ggplot(to.data[!is.na(to.data$mean.ba),], aes(x=Date)) +
  ...
  scale_y_continuous(breaks=c(1e8, 1.5e8, 2e8),
                     labels=c("1e8","1.5e8","2e8")) +
    theme_classic() +
    theme(text = element_text(size=10),
          panel.margin=unit(.05, "lines"),
        panel.border = element_rect(color = "black", fill = NA, size = 0.5),
...
leu_plot <- ggplot(anc_data, aes(x=Day, y=mean.leu, colour=Treatment)) + 
...
p.all <- ggplot(data = tdata.all, aes(x = Treatment, y = mean, fill = OTU)) + 
  geom_bar(stat="identity", color="black", size=0.01) +
...
g.doc <- ggplot(doc.b10, aes(x=Date, y=DOC..µmol.L.)) +
...
g.poc <- ggplot(poc.b10, aes(x=Date, y=C..µg.l.)) +
    geom_point(shape=20, na.rm=T, size=1) +    
    geom_line() + 
    xlab("") + 
    ylab(expression(paste("POC (", mu, "g  ", l^{-1},")"))) +
...
lm.doc.chl <- lm(DOC..µmol.L.~Chlorophyll..mg.m.., data=d)

d2 <- merge(poc.b10, chl.b10, by.x = "Date", by.y = "Date", all.x=FALSE, all.y=FALSE)

lm.poc.chl <- lm(C..µg.l.~Chlorophyll..mg.m.., data=d2)
...
m_poc <- lm(C..µg.l.~Chlorophyll..mg.m.., data=d2)
...
poc_vs_chl <- ggplot(d2, aes(x = Chlorophyll..mg.m.., y = C..µg.l.)) +
  geom_point(size = 1) +
...
m_doc <- lm.doc.chl <- lm(DOC..µmol.L.~Chlorophyll..mg.m.., data=d)
...
doc_vs_chl <- ggplot(d, aes(x = Chlorophyll..mg.m.., y = DOC..µmol.L.)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm", se = FALSE) +
    ylab(expression(paste("DOC (", mu, "mol  ", l^{-1},")"))) +
    xlab(expression(paste("Chlorophyll ", italic("a"), " (", mu, "g  ", l^{-1},")"))) +
