...
otutable <- fread('otu_table_mc2_DOM_even_from_biom.txt', skip=1)

meta <- fread('DOM.map.txt')
#meta$SamplingDate <- as.Date(meta$SamplingDate,format = '%m/%d/%Y')
#order meta according to MBL_library ID
idx_meta <- order(meta$'#SampleID', decreasing=FALSE)
meta_sorted <- meta[idx_meta]
#meta_sorted <- meta_sorted[(106:186),]
```
...
The OTU table has `r dim(otutable)[1]` OTUs to begin with. After removing any OTUs with "chloroplast" in the taxonomy, there are `r dim(otutable_bac)[1]` OTUs.
...
```{r, echo=FALSE}
# otutable_meta_DOM_samples <- as.data.frame(otutable_meta_DOM[,(1:18207)])
# otutable_bac.mds <- metaMDS(otutable_meta_DOM_samples, 
...
# Taxa plots
```{r set_up_taxa_table, echo=FALSE, eval=FALSE}
# # read in the data (had to manually change OTU column heading before reading in file)
# taxaL3 <- fread("cdout_DOM/taxa_plots/table_mc82638_sorted_L3.txt", skip=1)
# idx_chlor <- grep('Chloroplast', x = taxaL3$OTU.ID, invert=TRUE)
# taxaL3_bac <- taxaL3[idx_chlor]
# 
...
# }
# 
# taxaL3_bac_t <- tdt(taxaL3_bac)
# 
# #attach info from meta_Sorted, using library ID as the key
# setkey(meta_sorted,"#SampleID")
# setkey(taxaL3_bac_t,"OTU.ID")
# taxa_meta <- taxaL3_bac_t[meta_sorted]
# 
# # other formatting
# # convert to dataframe
# taxa_meta_df <- as.data.frame(taxa_meta)
# #make sure dates are read as dates
# taxa_meta_df$SamplingDate <- as.Date(taxa_meta_df$SamplingDate)
# 
# # get rid of all the metadata
# taxa <- taxa_meta_df[,-(171:180)]
# 
# #transpose the dataframe after removing metadata,m drop column 1
# taxa_t <- t(taxa[,-1])
# # use column 1 as column names for taxa_t
# colnames(taxa_t) <- taxa[,1]

```

```{r modify_taxa_names, echo=FALSE, eval=FALSE}
#functions to modify the taxa column
removeUptoUnderscore <- function(x){
  location <- regexpr('_', x)         # find the location in string of the first ';'
  newx <- substring(x, location+2)   # return everything after that location
  return(newx)
}

removeUptoPhylum <- function(x){
  location <- regexpr('p_', x)         # find the location in string of the first ';'
  newx <- substring(x, location+3)   # return everything after that location
  return(newx)
}

removeUptoClass <- function(x){
  location <- regexpr('c_', x)         # find the location in string of the first ';'
  newx <- substring(x, location+3)   # return everything after that location
  return(newx)
}

#create table just for functions
taxa_newnames <- taxa_t

#apply all three functions
rownames(taxa_newnames) <- removeUptoUnderscore(rownames(taxa_newnames))
rownames(taxa_newnames) <- removeUptoPhylum(rownames(taxa_newnames))
rownames(taxa_newnames) <- removeUptoClass(rownames(taxa_newnames))

#check after functions, assign new name
taxa_mod <- as.data.frame(taxa_newnames)
```

```{r sort_taxa_table, echo=FALSE, eval=FALSE}
# get row means
taxa_mod$average <- apply(taxa_mod, 1, mean)
# apply index to table to sort according to average relative abundance
idx <- order(taxa_mod$average, decreasing = TRUE)
taxa_sorted <- taxa_mod[idx, ]

#set the value for relative abundance below which taxa should be "other"
mythreshold<-0.005

# create a column with the threshold
taxa_sorted$highlow <- 
  ifelse( taxa_sorted$average > mythreshold, 
          rownames(taxa_sorted), 'Other' )
```

```{r manip_for_taxa_table, eval=FALSE, echo=FALSE}
#manipulate data and conver to long format for making taxa plots
#transpose again, need to get rid of last two columns (average and highlow)
taxa_sorted_t <- t(taxa_sorted[,-(82:83)])
#make highlow column into column names in transposed table
colnames(taxa_sorted_t) <- taxa_sorted[,83]
taxa_sorted_t_IDs <- cbind(taxa_meta_df$Experiment, taxa_meta_df$Day, taxa_meta_df$Treatment, taxa_sorted_t)

taxa_melt_IDs <- melt(taxa_sorted_t_IDs, id.vars=c("V1", "V2", "V3")
                    )

#rownames(taxa_sorted_t) <- taxa_meta_SWI$SamplingDate
taxa_sorted_melt <- melt(taxa_sorted_t)
colnames(taxa_sorted_melt) <- c("Sample", "Class", "RelativeAbundance")



# meta_melt_df <- cbind(taxa_meta_df$Experiment, taxa_meta_df$Day, 
#                       taxa_meta_df$Treatment)
# colnames(meta_melt_df) <- c("Experiment", "Day", "Treatment")
# meta_melt <- melt(meta_melt_df)
# colnames(meta_melt) <- c("Experiment", "Day", "Treatment")

cdata <- ddply(taxa_sorted_melt, c("Sample", "Class"), summarise,
...
```{r import_data_for_phyloseq, echo=FALSE, warning=FALSE, include=FALSE}
...
[linked phrase](http://www.ncbi.nlm.nih.gov/pubmed/22616650)
[linked phrase](https://books.google.com/books?id=cQANAwAAQBAJ&pg=PA26&lpg=PA26&dq=Collwellia&source=bl&ots=fQ7kEClmmF&sig=Lz1kFGSRtdNgWGzvBZY0fXHBZlE&hl=en&sa=X&ved=0CDUQ6AEwBWoVChMIp879iIH8xgIV1xuSCh1xpQQp#v=onepage&q=Collwellia&f=false) 
...
[linked phrase](http://aslo.net/lo/toc/vol_46/issue_7/1606.pdf)
3. The group contains some known psychrophiles
[linked phrase](http://www.ncbi.nlm.nih.gov/pubmed/16043709)
...
```{r taxa_plots_exp1, echo=FALSE, fig.cap="Changes in abundance (i.e. number of reads) for the twenty most prevalent OTUs over the course of the 10-day incubation period. OTUs are colored by Family."}
...
##September
Similar trends (declining Pelagibacteria and increasing Collwellia) occurred during the September experiment. Chloroplast sequences (grey blocks) were present at the beginning of the experiment but had largely disappeared by day 10. Polaribacter (Flavobacteria blocks) was prevalent in both Control and DOM+ carboys on Day 10. Carbon from phytoplankton die-off (as suggested by disappearing chloroplast sequences) could account for some of the changes that occurred in the Control carboys.

```{r taxa_plot_exp2, echo=FALSE, fig.cap="Changes in abundance (i.e. number of reads) for the twenty most prevalent OTUs over the course of the 10-day incubation period. OTUs are colored by Family."}
...
```{r taxa_plot_exp3, echo=FALSE, fig.cap="Changes in abundance (i.e. number of reads) for the twenty most prevalent OTUs over the course of the 10-day incubation period. OTUs are colored by Family."}
...
```{r taxa_plot_exp4, echo=FALSE, fig.cap="Changes in abundance (i.e. number of reads) for the twenty most prevalent OTUs over the course of the 10-day incubation period. OTUs are colored by Family."}
